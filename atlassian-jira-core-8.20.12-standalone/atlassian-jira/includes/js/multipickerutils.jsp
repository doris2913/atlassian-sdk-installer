...
function selectSingleGroup(event)
{
    ...
    const checkedInput = document.querySelector('input[name=userchecks]:checked');
    if (!checkedInput) {
        return window.close();
    }

    const customFieldId = document.getElementById("openElement").textContent.trim();
    const openerCustomFieldElem = opener.document.getElementById(customFieldId);
...
function selectSingleUser(event)
{
    ...
    const triggerEventName = document.getElementById("triggerEvent").textContent.trim();
    const customFieldId = document.getElementById("openElement").textContent.trim();
    const $openerCustomFieldElem = opener.AJS.$('#' + customFieldId);
...
{
    const radioElem = document.getElementById(id);
    if (!radioElem) {
        return;
    }
